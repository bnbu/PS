...
int makeSegTree(int start, int end, int node) {
    if (start == end)
...
int getSum(int start, int end, int node, int left, int right) {
...
void updateSegTree(int start, int end, int node, int idx, int diff) {
...
// 그리고 현재 노드의 값은 makeSegTree(start, mid, node * 2) + makeSegTree(mid + 1, end, node * 2 + 1)
